...
if(gid!='' && gid!=undefined && gid!=null){
	$.ajax({
		url:'/index.php/Admin/Goods/getFormatOption',
		data:{gid:gid},
		type:'post',
		async:false,
			...
			if(res['format_option']!=null){
				for(var i in res['format_option']){
					data2.push({
						...
						OldPrice : res['format_option'][i]['FO_Old_Price'],	//原价
...
//      bool = false;
	...
	for(var i=0;i<newlen;i++){
		...
		var val ={ id : "",title:titles, stock : "", OldPrice:"",CostPrice : "",price : "",weight:"",virtual:virtuals,iaddress:"" };
		//初始化
		if(num==1){
		...
		//点击刷新表
			...
			if( $(".option_id_" + ids).length>0){
				val ={
					...
					OldPrice : $(".option_OldPrice_" + ids+":eq(0)").val(),
		...
		hh += '<td class="danger"><input name="option_OldPrice[]" type="text" class="form-control option_OldPrice option_OldPrice_' + ids +'" " value="' +(val.OldPrice=='undefined'?'':val.OldPrice )+'"/></td>';
...
//			oBool = false;
//		}
		else if(!getPointBool($('input[name=goods_oldPrice]').val()) && !($('input[name=goods_oldPrice]').attr('disabled'))){
			alert('请输入正确的市场价（最多保留两位小数）！');
			oBool = false;
		}
